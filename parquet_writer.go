...
func (pw *ParquetWriter) convertToParquetValue(v interface{}, name string, index int) parquet.Value {
	...
	if t, ok := pw.cfg.FieldTypes[name]; ok {
		...
		case "int":
			...
			return parquet.ZeroValue(parquet.Int32).Level(0, 1, index)
		case "bigint":
			switch val := v.(type) {
			case int8:
				return parquet.Int64Value(int64(val)).Level(0, 1, index)
		...
		case "float":
			...
			return parquet.ZeroValue(parquet.Float).Level(0, 1, index)
		case "double":
			switch val := v.(type) {
			case int8:
			...
			case string:
				if i, err := strconv.ParseFloat(val, 64); err == nil {
					return parquet.DoubleValue(i).Level(0, 1, index)
				}
			}
			return parquet.ZeroValue(parquet.Double).Level(0, 1, index)
		case "boolean":
			switch val := v.(type) {
			case bool:
			...
			return parquet.BooleanValue(false).Level(0, 1, index)
...
func (pw *ParquetWriter) GetFieldType(name string) parquet.Node {
	if t, ok := pw.cfg.FieldTypes[name]; ok {
		...
		case "string":
			return parquet.Optional(
				parquet.Encoded(parquet.String(), &parquet.Plain),
			)
		...
		case "bytearray":
			return parquet.Optional(
				parquet.Encoded(parquet.Leaf(parquet.ByteArrayType), &parquet.Plain),
			)
